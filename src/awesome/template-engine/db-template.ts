...
export class DbTemplate {
  ...
  // 创建数据库
  getCreateDbSql(dbName: string): string {
    return `CREATE DATABASE IF NOT EXISTS ${dbName} CHARACTER SET ${this.character} COLLATE ${this.collate};`;
  }

  // 新增用户
  getAddUserSql(dbName: string, user: string, password: string): string {
    return `grant select on ${dbName}.* to ${user}@'%' identified by '${password}' WITH GRANT OPTION;`;
